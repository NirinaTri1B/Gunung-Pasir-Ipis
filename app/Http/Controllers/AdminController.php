...
class AdminController extends Controller
{
    ...
    public function index()
    {
        ...
        // PERBAIKAN 1: Hitung pengunjung berdasarkan registrasi yang masuk
        $jumlahPengunjung = Registrasi::count();
        ...
        // 5. LOGIKA GRAFIK SOS
        $sosLabels = ['Tersesat', 'Cedera / Luka-luka', 'Bahaya Lainnya'];
        $sosCounts = [
            Sos::where('jenis_sos', 'Tersesat')->count(),
            Sos::where('jenis_sos', 'Cedera / Luka-luka')->count(),
    ...
    public function filterDashboard(Request $request)
    {
        ...
        $pengunjung = Registrasi::whereBetween('created_at', [$mulai, $selesai])->count();
        ...
        $querySOS = Sos::whereBetween('created_at', [$mulai, $selesai]);

        $sosCounts = [
            (clone $querySOS)->where('jenis_sos', 'Tersesat')->count(),

            (clone $querySOS)->where('jenis_sos', 'Cedera / Luka-luka')->count(),

            (clone $querySOS)->where('jenis_sos', 'Bahaya Lainnya')->count(),
        ];
        ...
        return response()->json([
            ...
            'sos_labels'   => $sosLabels,
            'sos_counts'   => $sosCounts
        ]);
    }
    public function cetakLaporan(Request $request)
    {
        ...
        $queryReg = Registrasi::query();
        if ($mulaiInput && $selesaiInput) {
            $queryReg->whereBetween('created_at', [$mulaiInput . ' 00:00:00', $selesaiInput . ' 23:59:59']);
        }
        $totalPengunjung = $queryReg->count();
    ...
    public function ulasan()
    {
        ...
        $rataRata = round(Ulasan::avg('rating'), 1);
        return view('admin.ulasanadm', compact('ulasan', 'rataRata'));
    }
    ...
    {
        // Ambil semua konten, group berdasarkan 'grup'
        $konten = \App\Models\Konten::all()->groupBy('grup');
        return view('admin.konten', compact('konten'));
    }

    public function updateKonten(Request $request)
    {
        // Loop semua input yang dikirim form
        // Format input name: konten[key] = value
        foreach ($request->konten as $key => $value) {
            \App\Models\Konten::where('key', $key)->update(['value' => $value]);
        }
