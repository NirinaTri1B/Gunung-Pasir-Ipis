    ...
    @forelse($ulasan as $ul)
        <div class="card shadow-sm border-0 p-4 position-relative" style="border-radius: 12px; background-color: #ffffff;">
            ...
            <div class="position-absolute" style="top: 20px; right: 20px; display: flex; gap: 8px;">
                ...
                <form action="{{ route('admin.ulasan.toggle', $ul->id_ulasan) }}" method="POST">
                    @csrf
                    @method('PUT')

                    @if($ul->tampilkan == 1)
                        <button type="submit"
                                class="btn btn-sm btn-link text-warning p-0 font-weight-bold">
                            <i class="fas fa-eye-slash"></i> Sembunyikan
                        </button>
                    @else
                        <button type="submit"
                                class="btn btn-sm btn-link text-success p-0 font-weight-bold">
