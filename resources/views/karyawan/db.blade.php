    ...
    body.on('click', '.btn-validasi', function() {
        ...
        Swal.fire({
            ...
            didOpen: () => {
                ...
                const btnDone = document.getElementById('btn-done-v');

                // Flag: apakah sudah klik CEK KESESUAIAN
                let sudahDicek = false;
                // Flag: apakah sudah simpan riwayat (untuk ambil_kembali)
                let sudahSimpanRiwayat = false;

                // Disable btn-done-v dulu di awal
                btnDone.disabled = true;
                btnDone.style.opacity = '0.4';
                btnDone.style.cursor = 'not-allowed';

                // Tampilan awal kalau statusLama sudah ada
                if (statusLama !== 'proses' && statusLama !== 'sesuai') {
                    ...
                    // Kalau statusLama ambil_kembali, berarti sudah pernah dicek sebelumnya
                    sudahDicek = true;
                    // Tapi btn-done tetap disabled sampai dicek ulang
                } else if (statusLama === 'sesuai') {
                    hasilV.value = "Sesuai";
                    hasilV.style.color = "green";
                    sudahDicek = true;
                    // Kalau sudah sesuai, enable btn-done
                    btnDone.disabled = false;
                    btnDone.style.opacity = '1';
                    btnDone.style.cursor = 'pointer';
                }

                // CEK KESESUAIAN enable btn-done setelah diklik
                btnHit.addEventListener('click', () => {
                    let akhir = parseInt(document.getElementById('akhir_v').value) || 0;
                    sudahDicek = true;
                    sudahSimpanRiwayat = false; // reset kalau dicek ulang
                ...
                // SIMPAN RIWAYAT (ambil kembali)
                document.getElementById('btn-save-pelanggaran').addEventListener('click', () => {
                    fetch(`/karyawan/validasi-proses/${userId}`, {
                        method: "POST",
                        headers: { "Content-Type": "application/json", "X-CSRF-TOKEN": "{{ csrf_token() }}" },
                        body: JSON.stringify({
                            akhir: document.getElementById('akhir_v').value,
                            status_sampah: selectSolusi.value,
                            simpan_sementara: true,
                    ...
                    .then(res => {
                        if (res.success) {
                            sudahSimpanRiwayat = true;
                            Swal.fire('Tersimpan!', 'Riwayat dicatat. Tunggu pendaki ambil sampahnya.', 'info')
                            .then(() => {
                ...
                document.getElementById('btn-done-v').addEventListener('click', () => {
                    if (!sudahDicek) {
                        return Swal.fire('Perhatian!', 'Klik CEK KESESUAIAN dulu sebelum menyimpan.', 'warning');
                    }
                    ...
                    if (akhir < sampahAwalData && selectSolusi.value === 'ambil_kembali') {
                        return Swal.fire('Gagal', 'Simpan riwayat dulu dan tunggu pendaki ambil sampahnya!', 'warning');
                    }
                    ...
                    fetch(`/karyawan/validasi-proses/${userId}`, {
                        ...
                        body: JSON.stringify({
                            ...
                            selesai: true,
                            metode_pembayaran_denda: metodeDendaPilihan
                        })
                    })
                    .then(res => res.json())
                    .then(res => {
                        if (res.success) {
                            Swal.fire('Selesai!', 'Status pendaki diperbarui.', 'success')
                            .then(() => {
                                window.location.href = window.location.pathname;
                            });
                        }
