...
<div class="card">
    <div class="card" style="padding: 15px; border-radius: 12px;">
        ...
        {{-- Teks Penanda Posisi sesuai Revisi Dosen --}}
        <div style="font-size: 12px; color: #666; margin-bottom: 10px; display: flex; align-items: center; gap: 5px;">
            <i class="fas fa-map-marker-alt" style="color: #d62828;"></i>
            <span>Menampilkan jarak tempuh dan estimasi waktu dari lokasi Anda sekarang.</span>
        </div>
...
new L.GPX(gpxUrl, {
    ...
    if (layers.length > 0) {
        ...
        L.marker([endPoint.lat, endPoint.lng], {
            ...
            icon: L.divIcon({
                html: '<i class="fas fa-mountain" style="color: #007bff; font-size: 24px;"></i>',
                className: 'custom-div-icon',
                iconSize: [30, 30],
                iconAnchor: [15, 30]
            })

        }).addTo(map).bindPopup("Puncak");

        // ===== REALTIME TRACKING =====

        function updateRealtimeTracking(position) {
            ...
            // Hitung jarak ke puncak
            var distance = map.distance(
                [userLat, userLng],
                [endPoint.lat, endPoint.lng]
            ) / 1000;

            // Tampilkan jarak realtime
            document.getElementById('jarak').innerText =
                distance.toFixed(2);

            // Estimasi waktu
            // Asumsi kecepatan jalan 3 km/jam
            var estimasiMenit =
                Math.round((distance / 3) * 60);
