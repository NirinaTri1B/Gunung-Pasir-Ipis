...
<script>
    function cekNotifikasiSOS() {
        const excludedPaths = ['/karyawan/sos', '/karyawan/satwa'];
    ...
    // Fungsi untuk mulai polling, dipanggil SETELAH semua alert selesai
    function mulaiPolling() {
        cekNotifikasiSOS(); // Jalankan sekali dulu
        setInterval(cekNotifikasiSOS, 3000); // Baru mulai interval
    }
    ...
    @if(session('success'))
        Swal.fire({
            ...
            // Kasih jeda 500ms setelah success tutup, baru polling mulai
            setTimeout(mulaiPolling, 500);
        });
    @else
        mulaiPolling();
    @endif
